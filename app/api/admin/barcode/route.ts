  ...
  try {
    ...
    if (matchingVolume) {
      ...
      for (let page = 0; page < pages; page++) {
        ...
        issues.forEach((issue: Record<string, unknown>) => {
          ...
          allIssues.push({
            ...
            category: "fumetti",
            imageUrl: img?.super_url ?? img?.medium_url ?? img?.small_url ?? volImg?.super_url ?? volImg?.medium_url ?? "",
            publisher: pub?.name ?? "",
            year: coverDate,
            found: true,
          });
      ...
      return [{
        ...
        category: "fumetti",
        imageUrl: volImg?.super_url ?? volImg?.medium_url ?? volImg?.small_url ?? "",
        publisher: pub?.name ?? "",
    ...
    return (issueData.results ?? [])
      ...
      .map((item: Record<string, unknown>) => {
        ...
        return {
          ...
          category: "fumetti",
          imageUrl: img?.super_url ?? img?.medium_url ?? img?.small_url ?? "",
          year,
          found: true,
        } as BarcodeResult;
      });
